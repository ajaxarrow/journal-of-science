<?php
$data['title'] = 'Edit Assigned Author';
...
<div class="main-content-container">
    <div class="p-5">
        ...
        <p class="text-xl font-bold mt-4">Edit Assigned Author</p>
        <p class="text-sm">Edit assigned author to this article</p>
    </div>
    <hr/>
    <div class="p-5">
        <form action="<?php echo base_url(); ?>admin/article/<?php echo $id; ?>/author/update/<?php echo $articleauthor['id']; ?>" method="POST">
				<div class="my-4">
						<p class="font-medium mb-2">Author</p>
						<select name="author_id" class="select w-full h-[45px]">
								<?php foreach ($authors as $author): ?>
										<option value="<?php echo $author['author_id']; ?>" <?php echo ($articleauthor['authid'] == $author['author_id']) ? 'selected' : ''; ?>><?php echo $author['author_name']; ?></option>
								<?php endforeach; ?>
						</select>
